    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6">
        <div class="bg-body-bg rounded-xl border border-body-border p-6 md:p-10">
            <div class="flex gap-3 mb-6">
                <a href="{{ route('terms') }}" class="text-sm font-medium bg-body-bg text-body-muted px-3 py-1 rounded-full border border-body-border hover:text-primary transition">Risk Warning</a>
                <a href="{{ route('privacy') }}" class="text-sm font-medium bg-primary-subtle text-primary px-3 py-1 rounded-full hover:bg-primary hover:text-white transition">Privacy Policy</a>
            </div>
            ...
            <article class="prose prose-sm max-w-none text-body-muted leading-relaxed space-y-6">
                ...
                <p>Information necessary to verify your identity, such as an identification card, passport, or driver's license. This also includes background information we receive about you from public records, or from other entities not affiliated with {{ $settings->site_name }}.</p>
                ...
                <p>{{ $settings->site_name }} uses personal information only as required, in order to provide quality service and security to you. This information helps improve services, customize the browsing experience, and enables us to inform you of additional products, services, or promotions relevant to you and the products and services you need, as well as your consent for use of such data.</p>
                <p>If you do not want to receive information of this nature for any reason, please contact us at: <a href="mailto:{{ $settings->contact_email }}" class="text-primary hover:underline">{{ $settings->contact_email }}</a></p>
                <p>Although you are not required to provide {{ $settings->site_name }} with any of the personal information that we may request of you, please note that failure to do so, could result in {{ $settings->site_name }} being unable to open your account, or provide you with the service you need.</p>
                ...
                <p>Any personal information you provide us will be treated as confidential, shared only within {{ $settings->site_name }} and its affiliates, and will not be disclosed to any third party, except under any regulatory or legal proceedings.</p>
                ...
                <p>{{ $settings->site_name }} may work with affiliates and business introducers who refer new clients to the platform. If you were referred by an affiliate, that affiliate may have access to limited account information (such as your registration status). You consent to this sharing when you open an account through an affiliate link.</p>
                <p>If you are interested in our referral program, contact us at <a href="mailto:{{ $settings->contact_email }}" class="text-primary hover:underline">{{ $settings->contact_email }}</a>.</p>

                <h4 class="text-lg font-semibold text-body-text">Non-affiliated third parties</h4>
                <p>{{ $settings->site_name }} does not sell, license, lease, or otherwise disclose personal information to third parties, except as described in this privacy statement.</p>
                <p>{{ $settings->site_name }} reserves the right to disclose information as necessary to credit reporting or collection agencies as reasonably required in order to provide services to you.</p>
                <p>In order to help us improve our services, {{ $settings->site_name }} may involve third parties to help carry out certain internal functions, such as account processing, fulfillment, client service, client satisfaction surveys, or other data collection activities relevant to our business. Use of the shared information may also be used to provide professional, legal, or accounting advice to {{ $settings->site_name }}. Use of shared information is strictly limited to the performance of the above, and is not permitted for any other use. All third parties with which {{ $settings->site_name }} shares personal information are required to protect this information in accordance with all relevant legislation, and in a manner similar to the way {{ $settings->site_name }} protects the same. {{ $settings->site_name }} will not share personal information with third parties.</p>
                ...
                <p>{{ $settings->site_name }} reserves the right to disclose personal information to third parties where required by law, regulatory, law enforcement, or other government authority of a competent jurisdiction in order to protect our rights, and/or to comply with such legal proceedings. Such disclosure shall occur on a 'need-to-know' basis, unless otherwise instructed by a regulatory, or other government authority. Under such circumstances, {{ $settings->site_name }} shall expressly inform the third party regarding the confidential nature of the information.</p>

                <h4 class="text-lg font-semibold text-body-text">Restriction of responsibility</h4>
                <p>{{ $settings->site_name }} is not responsible for the privacy policies, or the content of sites it links to, and has no control of the use or protection of information provided by clients or collected by those sites. Whenever a client chooses to link to a co-branded website, or to a linked web site, this client may be asked to provide proof of registration or other information. Please note that such information is recorded by a third party, and will be governed by the privacy policy of that third party.</p>

                <h4 class="text-lg font-semibold text-body-text">Use of "COOKIES"</h4>
                <p>{{ $settings->site_name }} uses cookies in order to secure your trading activities, and to enhance the performance of the {{ $settings->site_name }} website. Cookies used by {{ $settings->site_name }} do not contain personal information or other sensitive information.</p>
                <p>{{ $settings->site_name }} may share website usage statistics with reputable advertising companies, and with its affiliated marketing companies. The information collected by the advertising company is not personally identifiable.</p>
                <p>To administer and improve the {{ $settings->site_name }} website, we may use third parties to track and analyze usage and statistical volume information. The third party may use cookies to track behavior, and may set cookies on {{ $settings->site_name }}'s behalf. These cookies do not contain any personally identifiable information.</p>

                <h4 class="text-lg font-semibold text-body-text">Privacy Statement Updates</h4>
                <p>From time to time, {{ $settings->site_name }} may update this Privacy Statement. In the event that {{ $settings->site_name }} materially changes this Privacy Statement, including how we collect, process or use your personal information, the revised Privacy Statement will be posted to the website. The client agrees to accept posting of a revised Privacy Statement electronically on the website as actual notice to him or her. Any dispute over our Privacy Statement is subject to this notice, and our Customer Agreement. {{ $settings->site_name }} encourages clients to periodically check back and review this policy, so that they always will know what information {{ $settings->site_name }} collects, how {{ $settings->site_name }} uses it, and to whom {{ $settings->site_name }} may disclose it.</p>
