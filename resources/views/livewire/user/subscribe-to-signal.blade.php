    ...
    @if ($hasSubscribe)
        ...
        <div class="text-center py-4">
            <div class="w-16 h-16 mx-auto bg-blue-500/10 rounded-full flex items-center justify-center mb-4">
                @include('components.icons.check-circle', ['class' => 'w-10 h-10 text-blue-400'])
            </div>
        ...
        <form wire:submit.prevent="subscribe" class="space-y-4">
            <div>
                ...
                <select wire:model="planId" wire:change="calculate"
                    class="w-full bg-[#1C2127] border border-[#2A2F36] rounded-lg px-4 py-2.5 text-[#E8EAED] focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">
