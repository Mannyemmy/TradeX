...
<div class="space-y-6">
    ...
    <x-admin.page-header title="Edit Withdrawal #{{ $withdrawal->id }}" subtitle="Update withdrawal details or backdate the transaction">
        <x-slot name="actions">
            <a href="{{ url()->previous() }}"
    ...
    <div class="max-w-xl">
        <x-admin.card>
            <form action="{{ route('admin.withdrawals.update', $withdrawal->id) }}" method="POST" class="space-y-5">
                @csrf
                ...
                <x-admin.form-group label="Payment Method" for="payment_mode">
                    <input type="text" id="payment_mode" name="payment_mode"
                           value="{{ old('payment_mode', $withdrawal->payment_mode) }}"
                           class="w-full bg-surface-card border border-border rounded-lg px-3.5 py-2.5 text-sm text-content focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors">
                </x-admin.form-group>
                ...
                <x-admin.form-group label="Status" for="status" :required="true">
                    <select id="status" name="status" required
                        ...
                        <option value="Processed" {{ old('status', $withdrawal->status) === 'Processed' ? 'selected' : '' }}>Processed</option>
                ...
                <x-admin.form-group label="Date Created (Backdate)" for="created_at"
                    ...
                    <input type="datetime-local" id="created_at" name="created_at"
                           value="{{ old('created_at', $withdrawal->created_at ? \Carbon\Carbon::parse($withdrawal->created_at)->format('Y-m-d\TH:i') : '') }}"
                           class="w-full bg-surface-card border border-border rounded-lg px-3.5 py-2.5 text-sm text-content focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors">
                </x-admin.form-group>
