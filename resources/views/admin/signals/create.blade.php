...
@section('content')
    ...
    <x-admin.page-header title="Create Signal">
        <x-slot name="actions">
        ...
        <x-admin.card>
            <form action="{{ route('signal.store') }}" method="POST">
                ...
                <x-admin.form-group label="Entry Price" class="mb-4" required>
                    <input type="number" name="entry_price" value="{{ old('entry_price') }}" step="0.01" class="w-full bg-surface-card border border-border rounded-lg px-3 py-2 text-sm text-content focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" required>
                </x-admin.form-group>

                <x-admin.form-group label="Take Profit" class="mb-4" required>
                    <input type="number" name="take_profit" value="{{ old('take_profit') }}" step="0.01" class="w-full bg-surface-card border border-border rounded-lg px-3 py-2 text-sm text-content focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" required>
                </x-admin.form-group>

                <x-admin.form-group label="Stop Loss" class="mb-4" required>
                    <input type="number" name="stop_loss" value="{{ old('stop_loss') }}" step="0.01" class="w-full bg-surface-card border border-border rounded-lg px-3 py-2 text-sm text-content focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" required>
                </x-admin.form-group>

                <x-admin.form-group label="Leverage" class="mb-4" required>
                    <input type="number" name="leverage" value="{{ old('leverage') }}" class="w-full bg-surface-card border border-border rounded-lg px-3 py-2 text-sm text-content focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" required>
                </x-admin.form-group>
