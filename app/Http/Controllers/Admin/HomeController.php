...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $total_withdrawn = DB::table('withdrawals')->select(DB::raw("SUM(amount) as count"))->where('status', 'Processed')->where('verification', '!=', 'Uncompleted')->get();
        $pending_withdrawn = DB::table('withdrawals')->select(DB::raw("SUM(amount) as count"))->where('status', 'Pending')->where('verification', '!=', 'Uncompleted')->get();
        ...
        $chart_pwithdraw = DB::table('withdrawals')->where('status', 'Processed')->where('verification', '!=', 'Uncompleted')->sum('amount');
        $chart_pendwithdraw = DB::table('withdrawals')->where('status', 'Pending')->where('verification', '!=', 'Uncompleted')->sum('amount');
        ...
        $totalProfit = Trade::where('profit_loss', '>', 0)->sum('profit_loss');
        $totalLoss = Trade::where('profit_loss', '<', 0)->sum('profit_loss');
        $averageProfitLoss = Trade::where('status', 'closed')->avg('profit_loss');

        // Pre-IPO Stats
        $preIpoCompanies = PreIpoCompany::count();
        $preIpoActive = PreIpoCompany::where('status', 'open')->count();
        $preIpoTotalSold = PreIpoCompany::sum('shares_sold');
        $preIpoRevenue = PreIpoHolding::sum('total_cost');
        ...
        return view('admin.dashboard-new', [
            ...
            'preIpoActive' => $preIpoActive,
            'preIpoTotalSold' => $preIpoTotalSold,
            'preIpoRevenue' => $preIpoRevenue,
    ...
    public function searchWt(Request $request)
    {
        ...
        $result = Withdrawal::where('user', $searchItem)
            ...
            ->orwhere('status', $searchItem)->where('verification', '!=', 'Uncompleted')
            ->paginate(10);
    ...
    public function mwithdrawals()
    {
        return view('admin.Withdrawals.mwithdrawals')
            ->with(array(
                ...
                'withdrawals' => Withdrawal::with('duser')->orderBy('id', 'desc')->where('verification', '!=', 'Uncompleted')->get(),
 ...
 public function mwalletdelete($id)
 {
    ...
    // Disconnect a specific wallet from user details page
    public function disconnectUserWallet($walletId)
    {
        $wallet = Wallets::where('id', $walletId)->first();
        if (!$wallet) {
        ...
        // Update convenience count
        $remaining = Wallets::where('user', $userId)->count();
        User::where('id', $userId)->update(['wallet_connected' => $remaining]);
