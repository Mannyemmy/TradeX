...
class InvPlanController extends Controller
{
    ...
    // Edit user investment plan (backdate + edit amount/status)
    public function editInvestment(int $id)
    {
        $investment = User_plans::with('dplan', 'duser')->findOrFail($id);
        $title = 'Edit Investment #' . $id;
        return view('admin.investments.edit', compact('investment', 'title'));
    }

    public function updateInvestment(Request $request, int $id)
    {
        $request->validate([
        ...
        $investment = User_plans::findOrFail($id);

        $data = [
            'amount'      => $request->amount,
            'active'      => $request->active,
            'expire_date' => $request->expire_date ?? $investment->expire_date,
            'updated_at'  => now(),
        ];
        if ($request->filled('created_at')) {
            $data['created_at'] = Carbon::parse($request->created_at);
        }
        DB::table('user_plans')->where('id', $investment->id)->update($data);
